...
def download_clip(video_identifier, output_filename,
    ...
    start_time_date = datetime.timedelta(seconds=start_time)
    # start_time = datetime.datetime.strptime(str(start_time), '%H:%M:%S')
    # print(start_time_date)
    
    
    # s_t = max(datetime.datetime.strptime('00:00:00', '%H:%M:%S'), start_time-datetime.datetime.strptime('00:00:03', '%H:%M:%S'))
    if end_time>0:
        end_time_date = datetime.timedelta(seconds=end_time)
        # end_time_date = datetime.timedelta(seconds=end_time)
        # end_time = datetime.datetime.strptime(str(end_time), '%H:%M:%S')
        command = ['ffmpeg',
                '-i', '"/home/josmy/kinetics-download-prep/{}"'.format(direct_download_url),
                '-ss', str(start_time_date),
                '-t', str(end_time_date - start_time_date),
                '-c:v', 'libx264',
                '-c:a', 'copy',
                '"%s"' % output_filename
                ]
        ...
        command = ['ffmpeg',
                    '-i', '"/home/josmy/kinetics-download-prep/{}"'.format(tmp_filename),
                    '-ss', str(datetime.timedelta(seconds=max(0, start_time - 3))),
                    '-t', str(datetime.timedelta(seconds=6)),
                    '-c:v', 'libx264',
                    '-c:a', 'copy',
